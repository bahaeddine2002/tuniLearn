    ...
    return (
      <nav className="bg-blue-900 text-white p-4 flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          ...
          <Link href="/teacher/profile" className="px-3 py-1 rounded bg-white text-blue-900 font-semibold hover:bg-blue-200 transition">My Profile</Link>
          <Link href="/teacher/courses">My Courses</Link>
          <Link href="/teacher/create-course">Create Course</Link>
          <Link href="/courses">All Courses</Link>
          <Link href="/teacher/profile" title="Profile">
            {user.profileImageUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img 
                src={user.profileImageUrl} 
                alt="Profile" 
                className="w-8 h-8 rounded-full border-2 border-white hover:border-blue-400 transition"
              />
            ) : (
              <span className="w-8 h-8 flex items-center justify-center rounded-full bg-blue-700 text-white font-bold">P</span>
            )}
          </Link>
          <button onClick={handleLogout} className="ml-4 text-red-400 hover:text-red-300">
            Logout
